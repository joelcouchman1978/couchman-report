-- Build per-game rates then aggregate with count n and stddevs
...
base AS (
  SELECT
    ...
    COALESCE(f.minutes_played, 0.0) AS minutes,
    COALESCE(f.tackles, 0.0) AS tackles,
    COALESCE(f.missed_tackles, 0.0) AS missed_tackles,
    COALESCE(f.run_metres, 0.0) AS run_metres,
    COALESCE(f.post_contact_metres, 0.0) AS post_contact_metres,
    COALESCE(f.penalties_conceded, 0.0) AS penalties_conceded,
    COALESCE(f.carries, 0.0) AS carries,
    COALESCE(f.line_breaks, 0.0) AS line_breaks,
    COALESCE(f.errors, 0.0) AS errors
...
-- Toby CSV (metric,value,ci_low,ci_high,note)
;
...
-- Ryan CSV (metric,value,ci_low,ci_high,note)
;

COPY (
  SELECT 'Metres/carry', ROUND(mpc,2),
         ROUND(mpc - 1.96*sd_mpc/NULLIF(SQRT(n),0),2),
         ROUND(mpc + 1.96*sd_mpc/NULLIF(SQRT(n),0),2),
         'Carry efficiency'
    FROM agg WHERE lower(player_name) LIKE lower('%Ryan Couchman%')
  UNION ALL SELECT 'Line breaks/80', ROUND(lb80,2),
         ROUND(lb80 - 1.96*sd_lb80/NULLIF(SQRT(n),0),2),
         ROUND(lb80 + 1.96*sd_lb80/NULLIF(SQRT(n),0),2),
         'Volatile small-sample'
    FROM agg WHERE lower(player_name) LIKE lower('%Ryan Couchman%')
  UNION ALL SELECT 'Tackle eff. %', ROUND(tack_eff_pct,1),
         ROUND(tack_eff_pct - 1.96*sd_tack_eff_pct/NULLIF(SQRT(n),0),1),
         ROUND(tack_eff_pct + 1.96*sd_tack_eff_pct/NULLIF(SQRT(n),0),1),
         'Role-adjusted'
    FROM agg WHERE lower(player_name) LIKE lower('%Ryan Couchman%')
  UNION ALL SELECT 'Errors/80', ROUND(errors80,2),
         ROUND(errors80 - 1.96*sd_errors80/NULLIF(SQRT(n),0),2),
         ROUND(errors80 + 1.96*sd_errors80/NULLIF(SQRT(n),0),2),
         'Stability'
    FROM agg WHERE lower(player_name) LIKE lower('%Ryan Couchman%')
  UNION ALL SELECT 'Run metres/80', ROUND(runm80,1),
         ROUND(runm80 - 1.96*sd_runm80/NULLIF(SQRT(n),0),1),
         ROUND(runm80 + 1.96*sd_runm80/NULLIF(SQRT(n),0),1),
         'Carries incl. returns'
    FROM agg WHERE lower(player_name) LIKE lower('%Ryan Couchman%')
