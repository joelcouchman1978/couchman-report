COPY (
  ...
  SELECT 'metric','value','ci_low','ci_high','note'
  UNION ALL
  SELECT 'Tackles/80',
         ROUND(tackles80,1),
         ROUND(tackles80 - 1.96*sd_tackles80/NULLIF(SQRT(n),0),1),
         ROUND(tackles80 + 1.96*sd_tackles80/NULLIF(SQRT(n),0),1),
         'Work-rate; adjusted for minutes'
  ...
  UNION ALL SELECT 'Errors/80',
         ...
         'Stability'
    FROM agg WHERE lower(player_name) LIKE lower('%Toby Couchman%')
) TO 'data/metrics_toby.csv' WITH (DELIMITER ',');
...
COPY (
  ...
  SELECT 'metric','value','ci_low','ci_high','note'
  UNION ALL SELECT 'Metres/carry', ROUND(mpc,2),
         ROUND(mpc - 1.96*sd_mpc/NULLIF(SQRT(n),0),2),
         ROUND(mpc + 1.96*sd_mpc/NULLIF(SQRT(n),0),2),
         'Carry efficiency'
  ...
  UNION ALL SELECT 'Run metres/80', ROUND(runm80,1),
         ...
         'Carries incl. returns'
    FROM agg WHERE lower(player_name) LIKE lower('%Ryan Couchman%')
) TO 'data/metrics_ryan.csv' WITH (DELIMITER ',');
